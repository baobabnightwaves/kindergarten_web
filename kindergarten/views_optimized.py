# kindergarten/views_optimized.py
"""
Optimized versions of key view functions.
These functions replace the originals in views.py to improve database query performance.
"""
...
def home_optimized(request):
    """Optimized home page with aggregated queries"""
    today = date.today()
    
    # Optimize: Use aggregate to combine queries
...
def student_detail_optimized(request, pk):
    """Optimized student detail with select_related and prefetch_related"""
    # Optimize: Use select_related for group and teacher
    student = get_object_or_404(
        Student.objects.select_related('group', 'group__teacher'), 
        pk=pk
    )
    
    # Optimize: Use select_related for parent info
    parents = StudentParent.objects.filter(student=student).select_related(
        'parent', 'parent__user'
    )
    
    # Optimize: Use select_related for teacher who noted attendance
...
def group_detail_optimized(request, pk):
    """Optimized group detail with prefetch_related for attendance"""
    # Optimize: Use select_related for teacher
    group = get_object_or_404(
        Group.objects.select_related('teacher', 'teacher__user'), 
        pk=pk
    )
    
    # Optimize: prefetch today's attendance to avoid N+1 queries
    ...
    # Get today's attendance with related data
...
def parent_list_optimized(request):
    """Optimized parent list with efficient filtering"""
    # Optimize: Use select_related for user
    parents = Parent.objects.select_related('user').all()
    
    # For teachers - only parents of their students
...
def parent_detail_optimized(request, pk):
    """Optimized parent detail with nested select_related"""
    # Optimize: Use select_related for user
    parent = get_object_or_404(Parent.objects.select_related('user'), pk=pk)
    
    # Optimize: Use select_related for student, group, and teacher
...
def student_list_optimized(request):
    """Optimized student list with efficient filtering"""
    ...
    if request.user.groups.filter(name='Воспитатели').exists() and hasattr(request.user, 'teacher_profile'):
        teacher = request.user.teacher_profile
        # Optimize: Filter directly by teacher instead of fetching groups first
...
def api_stats_optimized(request):
    """Optimized API stats with efficient queries"""
    from django.http import JsonResponse
    from datetime import timedelta
    
    today = date.today()
    
    # Optimize: Get attendance stats in one query
    attendance_stats = Attendance.objects.filter(attendance_date=today).aggregate(
        present=Count('pk', filter=Q(status=True)),
        absent=Count('pk', filter=Q(status=False))
    )
    ...
    # Optimize: Use select_related and annotate to avoid N+1
    ...
    # Optimize: Get attendance for last 7 days in one query
